...
public class ExtendedGameContactListener extends GameContactListener {
    ...
    private static final float GROUND_STICK_FORCE = 0.5f; // Force for ground sticking
    private static final float EDGE_TOLERANCE = 0.1f; // Tolerance for edge transitions
    ...
    private void handleSlopePreSolve(ControlledPlayer player, Contact contact) {
        ...
        if (normal.y > 0.2f) { // Y value is positive for upward-facing normals
            ...
            if (movingHorizontally) {
                ...
                if (Math.abs(normal.x) > 0.8f) { // Very steep normal => sharp edge
